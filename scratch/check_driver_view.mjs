...
async function testDriverView() {
  ...
  const email = `test_rider_${rand}@livraison.com`;
  const password = `rider_pwd_${rand}`;

  console.log("Creating test rider user...");
  const { data: signUpData, error: signUpError } = await supabase.auth.signUp({
    email,
    password,
    options: {
      data: { role: 'rider', name: 'Test Rider', phone: `+226 00 00 ${rand}`, vehicle: 'Moto' }
    }
  });
  ...
  console.log("Rider created with ID:", userId);
  ...
  console.log("Signed in as rider successfully. Querying livreurs_view...");
  ...
  console.log("Test rider user cleaned up.");
}
