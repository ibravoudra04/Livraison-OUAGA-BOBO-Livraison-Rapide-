...
-- --- FONCTION DE CALCUL DE DISTANCE HAVERSINE (OPTIMISATION) ---
-- Renvoie les livreurs d'une ville spécifique avec leur distance calculée côté serveur
...
BEGIN
  RETURN QUERY
  SELECT 
    v.id, v.name, v.vehicle, v.lat, v.lng, v.initial, v.contacts_count, v.subscription_paid, v.status, v.views_count, v.rating, v.city, v.created_at, v.selfie, v.phone_display, v.is_unlocked,
    -- Formule de Haversine pour la distance en kilomètres
    COALESCE(
      (6371 * acos( 
        cos(radians(client_lat)) * cos(radians(v.lat)) * cos(radians(v.lng) - radians(client_lng)) + 
        sin(radians(client_lat)) * sin(radians(v.lat)) 
      ))::numeric, 
    999.9) AS distance_km
  FROM public.livreurs_view v
  WHERE v.city = target_city
  ORDER BY distance_km ASC;
END;
$$;

GRANT EXECUTE ON FUNCTION public.get_nearby_riders(NUMERIC, NUMERIC, TEXT) TO anon, authenticated;
